...
export default function Step8({ nextStep }: { nextStep: () => void }) {
    ...
    const handleRegister = async () => {
        ...
        localStorage.setItem('alreadyPixDone', res?.status ? 'true' : 'false');
            ...
            <section className="flex flex-col h-screen items-center justify-start py-4 max-w-md mx-auto w-full bg-[#fdf8ea] font-semibold">
                ...
                <div className="w-full flex flex-col items-center mb-6 px-4">
                    ...
                    <div className="flex gap-4 w-full justify-center mb-4">
                        <button
                            className={`flex flex-col items-center justify-center w-1/2 py-4 rounded-xl transition-all duration-200 text-lg font-bold ${tab === 'cpf' ? 'bg-primary text-white' : 'bg-yellow-100 text-black/70'}`}
                            onClick={() => setTab('cpf')}
                            type="button"
                        >
                        ...
                        <button
                            className={`flex flex-col items-center justify-center w-1/2 py-4 rounded-xl transition-all duration-200 text-lg font-bold ${tab === 'phone' ? 'bg-primary text-white' : 'bg-yellow-100 text-black/70'}`}
                ...
                <div className="w-full flex flex-col items-center mb-8 px-4">
                    <div className="relative w-full max-w-md">
                        <input
                            ...
                            className="block w-full bg-transparent border-0 border-b-2 border-primary text-black text-lg px-0 pt-6 pb-2 focus:outline-none focus:ring-0 focus:border-primary placeholder-transparent"
                            placeholder=" "
